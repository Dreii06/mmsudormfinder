...
<div class="listappcontainer">
    <form action="/manager/detailsoccupant" method="POST" style=";width:95%;">
        ...
        <input type="text"  style="width:45%;margin-right:5%;" id="fname" name="first" value="{{ $details->first_name }} {{ $details->middle_name }} {{ $details->last_name }} {{ $details->suffix }}" class="inputapp" readonly>
        <input type="text" style="width:45%;" id="barangay" name="barangay" value="{{ $details->barangay }} {{ $details->street }} {{ $details->city }}, {{ $details->province }}" class="inputapp" readonly>
        ...
        <a href="{{ url('manager/listoccupants') }}"><button type="button" class="greenbutton" style="margin-top:0px;margin-right:5%;">BACK</button></a>
        <button type="submit" name="submit" value="DENY" onclick="denyapplicant()" class="greenbutton" style="margin-top:0px;margin-right:1%;color:white;background-color:#28a01b;">REMOVE</button>
    </form>
</div>
